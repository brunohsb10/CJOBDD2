...
ALTER TABLE INVOICE ADD CONSTRAINT FK_INVOICE_CUSTOMER 
    FOREIGN KEY (CUS_CODE) REFERENCES CUSTOMER(CUS_CODIGO);

ALTER TABLE PRODUCT ADD CONSTRAINT FK_PRODUCT_VENDEDOR 
    FOREIGN KEY (V_CODE) REFERENCES VENDEDOR(V_CODE);

ALTER TABLE LINE ADD CONSTRAINT FK_LINE_INVOICE 
    FOREIGN KEY (INV_NUMBER) REFERENCES INVOICE(INV_NUMBER);

ALTER TABLE LINE ADD CONSTRAINT FK_LINE_PRODUCT 
...
INSERT INTO VENDEDOR (V_CODE, V_NAME, V_CONTACT, V_AREACODE, V_PHONE, V_STATE, V_ORDER) VALUES
...
(25595, 'Rubicon Systems', 'Orton', '904', '458-0262', 'FL', 'Y');
...
('14-Q1/L3', '9.00-in. pwr. saw blade', '13/11/2007', 18, 12, 17.49, 0.00, 21344),
('1546-QQ2', 'Hrd. cloth, 1/4-in., 2x50', '15/01/2008', 15, 8, 39.95, 0.00, 23119),
('1558-QW1', 'Hrd. cloth, 1/2-in., 3x50', '15/01/2008', 23, 5, 43.99, 0.05, 23119),
...
('23114-AA', 'Sledge hammer, 12 lb.', '02/01/2008', 8, 5, 14.40, 0.05, NULL),
...
(1001, 10014, '16/01/2008'),
(1002, 10011, '16/01/2008'),
(1003, 10012, '16/01/2008'),
(1004, 10011, '17/01/2008'),
(1005, 10018, '17/01/2008'),
(1006, 10014, '17/01/2008'),
(1007, 10015, '17/01/2008'),
(1008, 10011, '17/01/2008');
...
SELECT COUNT(*) AS CLIENTES_SALDO_ALTO 
FROM CUSTOMER 
WHERE CUS_BALANCE > 500;


--18--Gere uma listagem de todas as compras feitas por clientes, utilizando como orientação 
--o resultado apresentado na Figura P7.16. (Sugestão: Utilize a cláusula ORDER BY para ordenar
--as linhas resultantes apresentadas na Figura P7.16.)

SELECT 
    I.CUS_CODE, 
    I.INV_NUMBER, 
    FORMAT(I.INV_DATE, 'dd-MMM-yy') AS INV_DATE, 
    P.P_DESCRIPT, 
    L.LINE_UNITS, 
    L.LINE_PRICE
FROM INVOICE I
...
--19-Utilizando como orientação o resultado apresentado na Figura P7.17, gere uma lista de compras de clientes, 
--incluindo os subtotais de cada número de linha de fatura. (Sugestão: Modifique o formato da consulta utilizada 
--para produzir a lista de compras de clientes do Problema 18, exclua a coluna INV_DATE e adicione o atributo 
--derivado (computado) LINE_UNITS * LINE_PRICE).
...
SELECT 
    ...
    CAST((L.LINE_UNITS * L.LINE_PRICE) AS DECIMAL(10,2)) AS Subtotal
FROM INVOICE I 
...
SELECT 
    C.CUS_CODE, 
    C.CUS_BALANCE, 
     CAST(SUM(L.LINE_UNITS * L.LINE_PRICE)AS DECIMAL(10,2)) AS [Total Purchases]
...
--(Em outras palavras, se a fatura do cliente basear-se em três produtos, um por número de linha (LINE_NUMBER), deve-se contar
--três compras de produtos. Observe que, nos dados da fatura original, o cliente 10011 gerou três faturas que continham um total de seis linhas,
--cada uma representando a compra de um produto.) Os valores de seus resultados devem coincidir com os apresentados na Figura P7.19."
...
--Os valores de seus resultados devem coincidir com os apresentados na Figura P7.20. Observe que a quantia média de compras é igual ao total de compras dividido pelo número
--de compras."
...
--23-Crie uma consulta para produzir as compras totais por fatura, gerando os resultados apresentados na Figura P7.21. O total da fatura é a soma das compras de produtos em
--LINE que correspondem à fatura (INVOICE).


SELECT 
    INV_NUMBER, 
    CAST(SUM(LINE_UNITS * LINE_PRICE) AS DECIMAL(10,2)) AS [Invoice Total]
FROM LINE
GROUP BY INV_NUMBER
ORDER BY INV_NUMBER;


--24-Utilize uma consulta para mostrar as faturas e totais de faturas conforme a Figura P7.22. (Sugestão: Agrupe por CUS_CODE).
...
SELECT 
    ...
    CAST(SUM(L.LINE_UNITS * L.LINE_PRICE) AS DECIMAL(10,2)) AS [Total Customer Purchases]
FROM INVOICE I JOIN LINE L 
ON I.INV_NUMBER = L.INV_NUMBER
GROUP BY I.CUS_CODE
ORDER BY I.CUS_CODE;
...
--26-Utilizando os resultados do Problema 25 como base, escreva uma consulta que gere o número total de faturas, o total de todas as faturas,
--o menor e maior valor de fatura, e o valor médio de todas as faturas. (Sugestão: Verifique o resultado da figura do Problema 25.)
--Seu resultado deve corresponder à Figura P7.24.


SELECT 
    COUNT(INV_NUMBER) AS [Total Invoices],
    CAST(SUM(Invoice_Total) AS DECIMAL(10,2)) AS [Total Sales],
    CAST(MIN(Invoice_Total) AS DECIMAL(10,2)) AS [Minimum Sale],
    CAST(MAX(Invoice_Total) AS DECIMAL(10,2)) AS [Largest Sale],
    CAST(AVG(Invoice_Total) AS DECIMAL(10,2)) AS [Average Sale]
FROM (
...
--27-Liste as características dos saldos dos clientes que fizeram compras durante o período de faturas — ou seja,
--dos clientes que aparecem na tabela INVOICE. Os resultados de sua consulta devem ser apresentados na Figura P7.25.
...
SELECT 
    CAST(MIN(CUS_BALANCE) AS DECIMAL(10,2)) AS [Minimum Balance],
    CAST(MAX(CUS_BALANCE) AS DECIMAL(10,2)) AS [Maximum Balance],
...
SELECT 
    CAST(SUM(CUS_BALANCE) AS DECIMAL(10,2)) AS [Total Balances],
    CAST(MIN(CUS_BALANCE) AS DECIMAL(10,2)) AS [Minimum Balance],
    CAST(MAX(CUS_BALANCE) AS DECIMAL(10,2)) AS [Maximum Balance],
    CAST(AVG(CUS_BALANCE) AS DECIMAL(10,2)) AS [Average Balance]
FROM CUSTOMER;
...
SELECT 
    CAST(SUM(CUS_BALANCE) AS DECIMAL(10,2)) AS [Total Balance],
    CAST(MIN(CUS_BALANCE) AS DECIMAL(10,2)) AS [Minimum Balance],
    CAST(MAX(CUS_BALANCE) AS DECIMAL(10,2)) AS [Maximum Balance],
    CAST(AVG(CUS_BALANCE) AS DECIMAL(10,2)) AS [Average Balance]
FROM CUSTOMER
...
--32-Crie uma consulta para produzir o resumo do valor dos produtos atualmente em estoque. Observe que o valor de cada produto é obtido pela multiplicação das unidades atualmente
--em estoque pelo preço unitário. Utilize a cláusula ORDER BY para obter a ordem apresentada na Figura P7.30.

SELECT 
    P_DESCRIPT, 
    P_QOH, 
    P_PRICE, 
    CAST((P_QOH * P_PRICE) AS DECIMAL(10,2)) AS Subtotal
FROM PRODUCT
ORDER BY 
    CASE 
        WHEN P_DESCRIPT LIKE 'Power%' THEN 1
        WHEN P_DESCRIPT LIKE '7.25%' THEN 2
        WHEN P_DESCRIPT LIKE '9.00%' THEN 3
        WHEN P_DESCRIPT LIKE 'Hrd. cloth, 1/4%' THEN 4
        WHEN P_DESCRIPT LIKE 'Hrd. cloth, 1/2%' THEN 5
        WHEN P_DESCRIPT LIKE 'B&D jigsaw, 12%' THEN 6
        WHEN P_DESCRIPT LIKE 'B&D jigsaw, 8%' THEN 7
        WHEN P_DESCRIPT LIKE 'B&D cordless%' THEN 8
        WHEN P_DESCRIPT LIKE 'Claw%' THEN 9
        WHEN P_DESCRIPT LIKE 'Sledge%' THEN 10
        WHEN P_DESCRIPT LIKE 'Rat-tail%' THEN 11
        WHEN P_DESCRIPT LIKE 'Hicut%' THEN 12
        WHEN P_DESCRIPT LIKE 'PVC%' THEN 13
        WHEN P_DESCRIPT LIKE '1.25%' THEN 14
        WHEN P_DESCRIPT LIKE '2.5%' THEN 15
        WHEN P_DESCRIPT LIKE 'Steel%' THEN 16
        ELSE 17 
    END;
	...
	SELECT 
    CAST(SUM(P_QOH * P_PRICE) AS DECIMAL(10,2)) AS [Total Inventory Value]
FROM PRODUCT;